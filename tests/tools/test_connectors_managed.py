...
Contracts:
- connect/reconnect mint ONE operation, block the turn via the callback, return per-leg
  outcomes; links live on the leg and never in the model result on a desktop session
...
- the watcher polls the gateway once per tick, transitions legs, settles on all-resolved
...
def test_card_legs_carry_the_account_id_the_mint_named():
    gw = GatewayFake(flips={"gmail": 2})
    cb = _desktop_callback()
    out = _run({"action": "connect", "connectors": ["gmail"]}, gw, callback=cb)
    (leg,) = cb.seen[0]["legs"]
    assert leg["connection_id"] == "ca_gmail_1"
    assert out["legs"][0]["connection_id"] == "ca_gmail_1"
...
def test_a_pending_row_moves_nothing_and_a_revoked_row_is_failed():
    ...
    # revoked is a failure, and a failed leg is unresolved: the deadline stamps it.
    assert out["settled_by"] == "deadline"
    assert out["legs"][0]["state"] == "not_connected"
...
def test_desktop_connect_mints_once_emits_the_card_and_returns_outcomes_without_urls():
    ...
    assert [m["connectors"] for m in gw.mints] == [("gmail", "notion")]  # one mint for every leg, up front
    (payload,) = cb.seen
    assert payload["op_id"] == out["op_id"]
    assert [t["name"] for t in payload["legs"]] == ["gmail", "notion"]
    assert all(t["kind"] == "connector" and t["action"] == "connect" for t in payload["legs"])
...
def test_desktop_connect_url_stays_on_the_live_operation_for_the_panel():
    ...
    snap = captured["op"].result()["legs"][0]
...
def test_the_watcher_reads_one_account_per_leg_per_tick_and_never_the_list():
    """The per-account route replaced the list walk: the watch loop asks for the accounts the mint
    named and nothing else, once each per tick, and stops reading a leg once it resolves."""
...
def test_respond_from_the_card_skips_a_leg_and_wakes_the_loop():
    gw = GatewayFake(flips={"gmail": 2})
...
def test_mint_failure_detail_survives_the_generic_list_copy():
    ...
    with patch("tools.connectors.operation.OPERATION_DEADLINE_SECONDS", 0.05):
        out = _run({"action": "connect", "connectors": ["gmail"]}, gw, callback=_desktop_callback(), tick=0.01)
    leg = out["legs"][0]
    assert leg["state"] == "not_connected"  # failed is unresolved; deadline stamped it
    assert leg["detail"] == "vendor: bad scope"
...
def test_reconnect_on_a_disconnected_leg_reinitiates():
    gw = GatewayFake(flips={"gmail": 2})
...
# ---------------------------------------------------------------------------
# settle races and terminal legs (verification findings P1-1, P1-7, P1-8)
# ---------------------------------------------------------------------------


def test_connected_read_on_a_failed_leg_is_ignored_not_an_error():
    ...
    assert out["legs"][0]["state"] in {"failed", "not_connected"}
    assert out["legs"][0]["detail"] == "denied"
...
def test_settle_reason_is_not_written_into_the_row_detail():
    ...
    assert out["settled_by"] == "deadline"
    assert out["legs"][0]["state"] == "not_connected"
    assert "detail" not in out["legs"][0]
...
# ---------------------------------------------------------------------------
# the watcher reads one account route per pending leg (NS-880)
# ---------------------------------------------------------------------------
...
def _one_initiated_leg():
    operation = op_module.ConnectionOperation([op_module.Leg("gmail", "connector", "connect")], session_key="s1")
    operation.transition("gmail", c.LegState.initiated, c.Actor.backend_watcher, connection_id="ca_1")
    return operation
...
def test_the_account_status_decides_the_state_and_who_caused_it(monkeypatch, status, state, actor):
    ...
    operation = _one_initiated_leg()
    managed._observe(_OneRead(status), operation)
    assert operation.leg("gmail").state.value == state
    if actor is None:
...
def test_the_account_status_reason_becomes_the_row_detail():
    ...
    assert operation.leg("gmail").detail == "the user revoked access"


def test_a_leg_the_mint_named_no_account_for_is_never_read():
...
def _two_initiated_legs():
    operation = op_module.ConnectionOperation(
        [op_module.Leg("gmail", "connector", "connect"), op_module.Leg("notion", "connector", "connect")],
        session_key="s1")
    for name, connection_id in (("gmail", "ca_1"), ("notion", "ca_2")):
        operation.transition(name, c.LegState.initiated, c.Actor.backend_watcher, connection_id=connection_id)
    return operation


def test_a_rate_limit_parks_every_live_leg_not_only_the_one_that_read():
    """The route's 429 budget is per principal, not per account: once one read is refused, the
    next leg's read in the same tick spends the same budget and would be refused too."""
    operation = _two_initiated_legs()
    ...
    assert all(t.next_read_at >= parked_at + 2.0 for t in operation.legs)


def test_a_skip_that_lands_during_the_read_drops_the_read_instead_of_raising():
    """The card's Skip on the RPC thread can resolve a leg while the watcher's read of it is in
    ...
    assert operation.leg("gmail").state == c.LegState.skipped
...
def test_an_account_the_gateway_does_not_know_moves_nothing_until_the_deadline():
    ...
    assert out["settled_by"] == "deadline" and out["legs"][0]["state"] == "not_connected"
...
def test_the_desktop_mint_names_the_return_leg_and_the_operation(monkeypatch, dev_server, expected):
...
def test_off_the_desktop_the_mint_names_no_return_leg():
