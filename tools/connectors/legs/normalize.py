"""Leg normalization and action validation for ``manage_connections``."""
...
_LEG_FIELDS = frozenset({"name", "mcp"})


def normalize_legs(raw: Any) -> Tuple[List[str], List[str], Optional[str]]:
    if raw is None:
        return [], [], None
    ...
    for item in raw:
        if isinstance(item, dict):
            unknown = sorted(set(item) - _LEG_FIELDS)
            if unknown:
                return [], [], (
                    f"unknown leg field(s) {', '.join(unknown)}: a leg is "
        ...
        if not name:
            return [], [], "every leg needs a non-empty 'name'."
...
def validate_action(action: str, managed: List[str], mcp: List[str]) -> Optional[str]:
    if action not in ALL_ACTIONS:
        return (
            ...
            "(legs {\"name\": ..., \"mcp\": true}); the rest apply to managed connectors. "
    ...
    if action in MCP_ACTIONS:
        if managed:
            return (
                f"'{action}' is an MCP action: every leg must carry \"mcp\": true "
        ...
        return (
            f"'{action}' is a managed-connector action; MCP legs ({', '.join(mcp)}) use "
            f"{', '.join(MCP_ACTIONS)}."
        )
    return None
